...
function GRB_SetFocusToNext(iRow, iLetter)
{
    ...
    if ( g_GRB_bAcross )
    {
        ...
        if ( !bLastLetterAcross && g_bSettings_CAGR_Navigation_WithinWord_SkipFilledSquares )
        {
            for ( iL = iLetter+1; iL < g_iGridWidth; iL++)
            {
                ...
                var bValidSquare = GRB_ForRowLetter_IsSquareValidForFocus(iRow, iL);
                if ( !bPlayerSet && bValidSquare )
                {
        ...
        { 
            ...
            {
                if ( !GRB_ForRowLetter_IsPlayerAnswerSet(iRow, iL)  && !GRB_ForRowLetter_isThisSquareABlackSquare(iRow, iL))
                {
                    GRB_MoveFocus(iRow, iL);
                    return;
                }
    ...
    if ( !bLastLetterDown && g_bSettings_CAGR_Navigation_WithinWord_SkipFilledSquares )
        {
            for ( iR = iRow+1; iR < g_iGridHeight; iR++)
            {
                ...
                var bValidSquare = GRB_ForRowLetter_IsSquareValidForFocus(iR, iLetter);
                if ( !bPlayerSet && bValidSquare )
                {
        ...
        { 
            ...
            {
                var bValidSquare = GRB_ForRowLetter_IsSquareValidForFocus(iR, iLetter);
                if ( !GRB_ForRowLetter_IsPlayerAnswerSet(iR, iLetter) && bValidSquare )
                {
                    GRB_MoveFocus(iR, iLetter);
                    return;
                }
    ...
    if ( !GRB_ForRowLetter_IsSquareValidForFocus(iNewRow, iNewLetter) )
    {
